...
const MeetingScheduler = ({ navigation}) => {
  ...
  const renderTimeSlots = () => {
    ...
    // Function to check if a time slot is past current time
    const isTimePast = (timeSlot) => {
      if (selectedDate !== new Date().toISOString().split('T')[0]) {
        return false; // Only check today's slots
      }
    ...
    return timeSlots.map((slot, index) => {
      ...
      return (
        <TouchableOpacity 
          ...
          onPress={() => !isDisabled && handleTimeSlotSelection(slot)}
          disabled={isDisabled}
        >
          <Text style={[
            styles.timeSlotText,
    ...
    <ScrollView>
      ...
      >
        ...
        <TouchableOpacity
          ...
          disabled={!selectedDate || !selectedTimeSlot || isBookingInProgress}
        >
